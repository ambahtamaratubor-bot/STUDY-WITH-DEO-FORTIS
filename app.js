...
// Restore elapsed from localStorage if returning mid-session
let elapsed=0;
let segStart=Date.now();
const _ps=localStorage.getItem('pomodoroState');
if(_ps){try{const _st=JSON.parse(_ps);if(_st.segStart&&_st.curSess===curSess&&_st.isBreak===isBreak){segStart=_st.segStart;elapsed=Math.floor((Date.now()-segStart)/1000);}}catch(e){}}
let segDone=false;
function tick(){
  if(segDone)return;
  ...
  localStorage.setItem('pomodoroState',JSON.stringify({elapsed,segStart,curSess,isBreak,cfg,activeSessionId:window.activeSessionId,sessionStartTime:window.sessionStartTime}));
  updateTimerBar();
  if(elapsed>=mt){
    segDone=true;
    clearInterval(interval);interval=null;
...
// Auto-start if: break, subsequent session, OR returning mid-session
const resuming=elapsed>0;
if(isBreak||curSess>1||resuming){
  running=true;
  if(!resuming)segStart=Date.now();
