...
function makeMarkdownToolbar(ta){
const toolbar=div({style:{display:'flex',gap:'6px',marginBottom:'8px',flexWrap:'wrap'}},[]);
var buttons=[{label:'B',prefix:'**',suffix:'**',fontWeight:'700',fontStyle:'normal'},{label:'I',prefix:'_',suffix:'_',fontWeight:'400',fontStyle:'italic'},{label:'U',prefix:'__',suffix:'__',fontWeight:'400',fontStyle:'normal'},{label:'H',prefix:'==',suffix:'==',fontWeight:'400',fontStyle:'normal'}];
buttons.forEach(function(btnConfig){
const handler=function(){const start=ta.selectionStart;const end=ta.selectionEnd;const selected=ta.value.substring(start,end);let newValue,newStart,newEnd;if(selected.length===0){newValue=ta.value.substring(0,start)+btnConfig.prefix+btnConfig.suffix+ta.value.substring(end);newStart=start+btnConfig.prefix.length;newEnd=newStart;}else{newValue=ta.value.substring(0,start)+btnConfig.prefix+selected+btnConfig.suffix+ta.value.substring(end);newStart=start;newEnd=end+btnConfig.prefix.length+btnConfig.suffix.length;}ta.value=newValue;ta.focus();ta.setSelectionRange(newStart,newEnd);};
const tb=btn(btnConfig.label,'btn-outline',handler,{style:{fontSize:'11px',padding:'4px 10px',fontWeight:btnConfig.fontWeight,fontStyle:btnConfig.fontStyle}});tb.onmousedown=function(e){e.preventDefault();};toolbar.append(tb);});
return toolbar;}
