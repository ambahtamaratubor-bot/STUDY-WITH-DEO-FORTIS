...
function landing(){
  ...
  nav.append(
    ...
    div({style:{display:'flex',gap:'32px',alignItems:'center'}},[
      ...
      btn('Sign Up','btn-gold',()=>go('signup'),{style:{padding:'8px 18px'}}),
    ])
  );
  ...
  (async()=>{
    ...
    if(pg&&pkgs&&pkgs.length){pkgs.forEach((pkg,i)=>{
...
function signup(){
  ...
  const sb2=btn('Create Account','btn-gold',async()=>{
    ...
    await sb.from('profiles').insert({id:data.user.id,email:emailI.value,full_name:nameI.value,status:'pending',plan:sel.name});
    wrap.innerHTML='';
